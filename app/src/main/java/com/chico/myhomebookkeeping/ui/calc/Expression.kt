        ...
        while (i < cleanCalculationLength) {
            ...
            if (cleanCalculation[i] == '(') {
                ...
                if (i + 1 < cleanCalculation.length) {
                    val functionsList =
                        listOf("arcco", "arcsi", "arcta", "cos", "sin", "tan", "ln", "log", "xp")
                    for (function in functionsList) {
                        ...
                        if (text.endsWith(function) && text.length != function.length) {
                            if (text[text.length - function.length - 1] !in "+-/*(^") {
                                cleanCalculation =
                                    cleanCalculation.subSequence(0, i - function.length + 1)
                                        .toString() +
                                            "*" + function + cleanCalculation.subSequence(
                                        i + 1,
                                        cleanCalculation.length
                                    ).toString()
                                cleanCalculationLength++
                                break
                            }
            ...
            while (i > 0) {
                ...
                if (cleanCalculation[i] == '!') {
                    ...
                    if (cleanCalculation[i - 1] == ')') {
                        // Remove the "!"
                        cleanCalculation =
                            cleanCalculation.substring(0, i) + cleanCalculation.substring(i + 1)
                        ...
                        cleanCalculation = cleanCalculation.substring(
                            0,
                            i
                        ) + ')' + cleanCalculation.substring(i + 1)
